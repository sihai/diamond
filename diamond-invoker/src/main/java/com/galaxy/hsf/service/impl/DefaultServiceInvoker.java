...
public class DefaultServiceInvoker extends AbstractServiceInvoker {
	 ...
	 * @param rpcProvider
	 */
	public DefaultServiceInvoker(ServiceRouter serviceRouter, RPCProvider rpcProvider) {
		this(serviceRouter, rpcProvider, DEFAULT_RETRY_TIMES);
	}
	 ...
	 * @param retryTimes
	 */
	public DefaultServiceInvoker(ServiceRouter serviceRouter, RPCProvider rpcProvider, int retryTimes) {
		super(serviceRouter, rpcProvider);
	...
	protected Object invoke0(HSFRequest request, List<String> addresses) throws HSFException {
		...
		for(; i < retryTimes && i < addresses.size(); i++) {
			try {
				...
				RPCProtocol rpcProtocol = rpcProvider.newRPCProtocol(serviceURL);
