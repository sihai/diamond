...
public class HSFContainer {
	...
	private static final Log logger = LogFactory.getLog(HSFContainer.class);
	
	private static final String BUNDLE_PROPERTY_FILE = "hsf_bundle.properties";
	
	private static final String CONTAINERCALLBACK_BUNDLE = "hsf.log.callback";
	
	private static final String CALLBACK_SERVICE = "com.taobao.hsf.log.CallbackService";
	
	private static final String CALLBACK_COMPONENT = "com.taobao.hsf.log.impl.CallbackComponent";
	
	private static final String HSF_LOGPATH_KEY = "HSF.LOG.PATH";
	
	private static BundleContext context = null;
	
	private static final String HSF_LIFECYCLE_FILE = "hsf_lifecycle_file.properties";
	
    private static final String  EXCLUDED_FILE     = "excluded.properties";
    
    private static final String EXCLUDED_PREFIX_PROP = "com.galaxy.hsf.container.excluded.prefixes";
	...
	private static void initLifecycleListeners() {
		try {
			Enumeration<URL> cUrls = HSFContainer.class.getClassLoader().getResources(HSF_LIFECYCLE_FILE);
			while(cUrls.hasMoreElements()){
				...
				try {
					...
					while((line = br.readLine()) != null){
						try {
							Class clazz = HSFContainer.class.getClassLoader().loadClass(line.trim());
			...
			System.err.println("获取HSF生命周期文件出错!  错误原因："+e.getMessage());
			e.printStackTrace();
		}
	}
	...
	private static void setExtenalClass() {
        ...
        if (excluded == null) {
            ...
            //  在hsf.container中
            String excludedPropertiesFile = System.getProperty("com.galaxy.hsf.container.excluded.prefixes.file");
            ...
            try {
                is = HSFContainer.class.getClassLoader().getResourceAsStream(excludedPropertiesFile);
				if (is != null) {
					properties.load(is);
					is.close();
				}
	...
	private static void startOSGI(String[] args) throws Exception {
		...
		// 在equinox已配置的hook上再增加HSFClassLoaderDelegateHook
		FrameworkProperties.setProperty("osgi.hook.configurators", "org.eclipse.osgi.internal.baseadaptor.BaseHookConfigurator," +
				...
				"com.galaxy.hsf.container.HSFClassLoaderDelegateHook");
		...
		File configDir = new File("hsf.configuration");
		if(configDir.exists()){
			deleteDir(configDir);
		}
	...
	private static String buildOSGIBundles(String[] args) throws Exception {
		...
		if (args == null || args.length == 0) {
			url = HSFContainer.class.getClassLoader().getResource("");
		} else {
			url = new File(args[0]).toURI().toURL();
		}
		String path = url.getPath();
		System.setProperty("HSF.CONTAINER.PATH", path);
	...
	private static void buildBundlesString(String parent , String child ,StringBuilder sb, boolean first) {
		...
		if(file.exists() ) {
			...
			if(first) {
				throw new RuntimeException("HSF运行必须的插件包不存在，错误原因： 目录 " + parent + " 下面不存在" + child + "目录!");
			}
		}
	}
	...
	public static void stop() {
		...
		try {
			...
			System.err.println("停止HSF容器时出现错误:"+e);
			e.printStackTrace();
		}
		try {
	...
	public static void addThirdContainerClassLoader(ClassLoader loader){
		HSFClassLoaderDelegateHook.addThirdClassLoader(loader);
	}
	...
	private static void initCallbackService() throws Exception {
		...
		for (int i = 0; i < bundles.length; i++) {
			...
			if(bundleName.equals(CONTAINERCALLBACK_BUNDLE)){
				...
				Method method = callbackComponent.getClass().getMethod("setHSFContainer", new Class<?>[]{Object.class});
				method.invoke(callbackComponent, new Object[]{new HSFContainer()});
				break;
			}
		}
	...
	private void initLogPath(){
        ...
        if (null == bizFileAppender) {
        	if(System.getProperty(HSF_LOGPATH_KEY) == null){
        		 System.setProperty(HSF_LOGPATH_KEY, "");
        	}
            return;
        }
        String bizLogDir = new File(bizFileAppender.getFile()).getParent();
        if (bizLogDir != null) {
			System.setProperty(HSF_LOGPATH_KEY, bizLogDir);
		} else {
        	if(System.getProperty(HSF_LOGPATH_KEY) == null){
       		 	System.setProperty(HSF_LOGPATH_KEY, "");
        	}
