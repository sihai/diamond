...
public class Logger {
	...
	public static final String LOGGER_NAME = "com.galaxy.diamond";
	
	private static final Log LOGGER = LogFactory.getLog(LOGGER_NAME);
	private static AtomicBoolean initialized = new AtomicBoolean(false);	//
	private static final String version = Constants.CURRENT_VERSION;
	...
	static public void init() {
		...
		// 使diamond的log4j配置生效(Logger, Appender)
		DOMConfigurator.configure(Logger.class.getClassLoader().getResource("diamond_log4j_" + version + ".xml"));

		// 设置log4j.xml中所有的Appender, 将这些appender输出的日志全部进行改变
		String bizLogDir = System.getProperty("DIAMOND.LOG.PATH");
